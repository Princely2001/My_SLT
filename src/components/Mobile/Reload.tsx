...
export default function Reload({ onBack, isMobile = false }) {
  ...
  if (showHistory) {
    return <ReloadHistory onBack={() => setShowHistory(false)} isMobile={isMobile} />;
  }
        ...
        >
          ...
          <Box sx={{ display: 'flex', flexDirection: 'row', gap: 3 }}>
            ...
            <Box sx={{
              ...
              <Box sx={{ mb: 2 }}>
                <Typography variant="body2" sx={{ fontWeight: 700, fontSize: '18px', color: colorScheme.secondaryAccent, mb: 1 }}>
                  Current Balance
                </Typography>
                <Typography variant="h5" sx={{ fontWeight: 700, color: colorScheme.accent }}>
                  {currentBalance}
                </Typography>
              </Box>
              ...
              >
                <Typography variant="body2" sx={{ fontWeight: 700, fontSize: '18px', color: colorScheme.accent, textTransform:'capitalize' }}>
                  Reload History
                </Typography>
              </Button>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 2 }}>
                {reloadBundles.map((bundle, index) => (
                  <Paper key={index} sx={{
                    ...
                    <Box>
                      <Typography variant="body2" sx={{ fontWeight: 700, fontSize: '18px', color: colorScheme.secondaryAccent }}>
                        {bundle.name}
                      </Typography>
                      <Typography variant="body2" sx={{ color: colorScheme.textSecondary, mt: 0.5 }}>
                        {bundle.validity}
                      </Typography>
                      <Typography variant="body2" sx={{ color: '#27ae60', fontWeight: 600, mt: 0.5 }}>
                        {bundle.bonus}
                      </Typography>
                    </Box>
                    <Box>
                      <Button
                        ...
                        onClick={() => alert(`Reloading ${bundle.name}`)}
                      >
